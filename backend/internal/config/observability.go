...
func DefaultObservabilityConfig() *ObservabilityConfig {
	return &ObservabilityConfig{
		...
		NewRelic: NewRelicConfig{
			...
			DebugLogging:              false, // disabled by default to avoid mixed log format
		},
...
func (c *ObservabilityConfig) Validate() error {
	if c.ServiceName == "" {
		return fmt.Errorf("setrvice name is required")
	}
	//validate log levels
	...
	if !validLevels[c.Logging.Level] {
		return fmt.Errorf("invalid logging level :%s (must be of : debug,info, warm, error)", c.Logging.Level)
	}

	//valide slow quer threshold
